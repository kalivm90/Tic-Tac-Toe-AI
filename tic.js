...
const gameController = (() => {
    ...
    const clearCounters = () => {
        totalCount = winCount = loseCount = 0;
        displayController.updateScoreboard(totalCount, winCount, loseCount)
        resetGame()
    }

    return { playRound, getGameOver, resetGame, updatePlayer, clearCounters}
})();
...
const displayController = (() => {
    ...
    startOverBtn.onclick = () => _resetGame();
    ...
    const _resetGame = () => {
        _resetBoard()
        gameController.clearCounters()
    }

    return { displayMessage, updateScoreboard}
